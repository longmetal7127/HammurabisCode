...
public class CommandsLogging {
  ...
  static {
    ...
    try {
      ...
      sequenceCommandIndexField =
          SequentialCommandGroup.class.getDeclaredField("m_currentCommandIndex");
  ...
  public static String getCommandName(Command command) {
    ...
    if (command instanceof WrapperCommand wrapperCommand) {
      try {
        ...
        // If it does modify getName(), then the else statement in the if-else chain covers that
    ...
    // Pain.
    // For all cases in this giant if-else chain, build a String representing the type and
    // subcommands of each command group.
    // Recurse as needed for nested command groups.
    // If the name of the command is modified, the command type is a WrapperCommand and thus
  ...
  public static void logRunningCommands() {
    DogLog.log("CommandScheduler/Running/.type", "Alerts");
    ...
    DogLog.log("CommandScheduler/Running/errors", interrupters);
  }
  ...
  public static void logRequiredSubsystems() {
    DogLog.log("CommandScheduler/Subsystems/.type", "Alerts");
    ...
    DogLog.log("CommandScheduler/Subsystems/infos", subsystems);
  }
}